#!/usr/bin/env python3
"""Fail-closed offline audit for the bounded a16-dev-p3a-fbm-r1 candidate."""
...
BASE = REPO / "out/candidates/a16-dev-p3a-omx-r1"
CONFIG = REPO / "configs/candidates/a16-dev-p3a-fbm-r1.json"
AUDIO_AUDIT_PATH = REPO / "scripts/audit-a16-dev-audio-r1.py"
SPEC = importlib.util.spec_from_file_location("audio_r1_audit_for_p3a_fbm_r1", AUDIO_AUDIT_PATH)
...
class Auditor(AUDIO_AUDIT.Auditor):
    def execute(self) -> None:
        ...
        if record(self.base / "x12-a16-dev-p3a-omx-r1.img") != {
            "path": str(self.base / "x12-a16-dev-p3a-omx-r1.img"),
            "size": self.cfg["base_candidate"]["size"],
            "sha256": self.cfg["base_candidate"]["sha256"],
        }:
            raise RuntimeError("exact tested OMX-r1 base identity changed")
        ...
        try:
            ...
            self.run(
                [sys.executable, str(REPO / "tools/sunxi_image_tool.py"), "verify",
                 str(self.candidate / "x12-a16-dev-p3a-fbm-r1.img")],
            ...
            if json.loads(base_lp.read_text()) != json.loads(candidate_lp.read_text()):
                raise RuntimeError("LP metadata/extents differ from exact OMX-r1")
            ...
            if record(self.base / "system_a.img")["sha256"] != record(images["system"])["sha256"]:
                raise RuntimeError("OMX-r1 system image changed")

            change = self.cfg["runtime_change"]
            old = self.candidate / "libfbm.omx-r1.so"
            new = points["vendor"] / "lib/libfbm.so"
            ...
            for item, expected, label in (
                ...
                if item["size"] != expected["size"] or item["sha256"] != expected["sha256"]:
                    raise RuntimeError(f"{label} libfbm identity changed")
            ...
            for revision in ("clang-r547379", "clang-r530567"):
                ...
                text = exact_command_output([
                    ...
                    "--start-address=0x391c", "--stop-address=0x3ccc", str(new),
                ])
                (self.audit / f"fbm-patched-{revision}.txt").write_text(text)
                for marker in (
                    "3934: f44f 40c0", "mov.w\tr0, #0x6000",
                    "3942: f001 eef6", "3946: b1b8", "3948: f8c8 0098",
                    "3a88: f8d4 0098", "3a90: b921", "3a92: f001 ee7e", "3a98: f8c4 0098",
                ):
                    if marker not in text:
                        raise RuntimeError(f"{revision} FBM ownership disassembly missing: {marker}")
            ...
            result = {
                "schema": 1,
                "candidate": record(self.candidate / "x12-a16-dev-p3a-fbm-r1.img"),
                ...
                "classification": "P3A_RC_A2_REPAIR_CANDIDATE_DEVELOPMENT_ONLY_NOT_R8_NOT_RELEASE",
                "filesystem": {
                    "e2fsck": "PASS_SYSTEM_VENDOR_PRODUCT_VENDOR_DLKM",
                    "system_byte_identical_to_omx_r1": True,
                    "system_tree_delta": {"added": [], "removed": [], "changed": []},
                    "vendor_tree_delta": vendor_delta,
                    "semantic_runtime_delta_count": 1,
                },
                "elf": {
                    "omx_r1": old_record, "p3a_fbm_r1": new_record,
                ...
                "disassembly": "PASS_MALLOC_0X6000_EXISTING_FREE_OWNERSHIP_UNCHANGED",
                "disassemblers": disassembly_records,
                "preserved_runtime": preserved,
                "avb_lp_outer": {
                    "system_vendor_vbmeta_system_vbmeta_vendor": "PASS",
                    "lp_metadata_and_extents": "EXACT_OMX_R1",
                    "sparse_raw_roundtrip": "PASS_BYTE_EXACT",
                    "imagewty_outer": "PASS",
                    "boot_kernel_vendor_dlkm_product": "BYTE_IDENTICAL_OMX_R1",
                },
                "vintf": {
                ...
                "governance": {
                    ...
                    "rc_a2": "PATCH_IMPLEMENTED_OFFLINE_CANDIDATE_BUILT_PHYSICAL_VALIDATION_PENDING",
                    "rc_b": "PHYSICAL_FAIL_EXACT_4K_CONTRACT_CAPTURED_COMPAT1B_IMPLEMENTATION_READY",
                    "p3b_main10": "NOT_AUTHORIZED",
                    "r8": "NOT_AUTHORIZED_NOT_BUILT",
                },
                "limitations": [
                    "No physical device action, ADB, flash or playback occurred.",
                    "RC-A2 is not physical PASS until one bounded hardware retest.",
