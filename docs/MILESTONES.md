...
当前关键节点：**Test8r2 仍是唯一稳定基线；Test9r1 已因 RRO 未被扫描而失败，Test9r2 单变量修正已离线通过并等待真机 official Google TV iPhone remote/text input 验收，Test9w1 不再使用。与此同时只推进 M8.0 只读 inventory，不制作 64 位候选。64 位迁移第一硬门槛是 H616/Mali-G31 的 64 位图形 same-process HAL；M8.INPUT 不以 UBOX Input 替代，Netflix/DRM 原厂与 Test8r2 基线必须在相关修改前建立。**
