...
## D-0008 — 稀疏格式化补救无效与黑盒调试边界

- 日期：2026-07-20
- 来源：实机二次物理烧录测试。
- 方法：使用 `img2simg` 转换后的稀疏 `super.img` 重新封装固件并上机刷写[cite: 15]。
- 结论：
  1. 进度依然死锁于 5%-10%，伴随 LED 高频闪烁 panic 报警[cite: 11]。
  2. 这从逻辑上排除了“单一稀疏格式兼容性”导致挂起的假设[cite: 15]。
  3. （推测）核心诱因为物理边界越界：重排的 `super.img` 总体积超出了底层引导程序分配的最大可用 eMMC 磁道边界[cite: 14]。
- 置信度：高（基于两次实机现象的高度一致性与对照组实验的排他性）。
- 影响：中止当前基于猜测的重打包行为。必须严格执行静态体积测量（验证修改包体积是否 $\le$ 原厂包）。若体积未超标，必须直接通过 UART 获取底层 Panic 异常栈，作为下一步修复的唯一依据[cite: 11, 14]。