...
- Test9r2 仍从 Test8r2 构筑，唯一相对 Test9r1 的变化是把相同 SHA-256 的 RRO 移至 `/system/system_ext/overlay`。system manifest 只有修正后的路径，官方 `vendor_dlkm` 不变；ext4/e2fsck、完整 AVB、super、IMAGEWTY 和 25 项单元测试套件通过。固件为 2,005,946,368 bytes，SHA-256 `27B54FB83E96D3863FAE2EF2718E8EC9ADDD863E5ED123082D5E6C8CA6FFFD52`。
- Test9r2 真机证明 system_ext RRO 生效，framework lookup 返回 `com.google.android.tv.remote.service`，provider 已绑定。原始 Remote Service 在 `getBondedDevices/getAddress` 因缺少 `android.permission.BLUETOOTH_CONNECT` 崩溃，`crashCount=2`，6466/6467 不监听；这是一项缺失的 Android 12+ default-permissions 产品集成，而不是 WLAN 或 iPhone 发现故障。
- 只在 userdata 临时授予 `BLUETOOTH_CONNECT` 并重新触发服务后，Remote Service 主进程稳定、6466/6467 监听、服务器证书生成，`_androidtvremote2._tcp` 以 `Pixel 3` 名称注册；约 18 分钟后的最终复核仍无新崩溃。`BLUETOOTH_SCAN` 和 `BLUETOOTH_ADVERTISE` 仍未授予，证明当前 Remote v2 路径无需扩大这两项权限。
- 官方 Google TV iPhone 应用随后完成 TLS 1.2 配对、控制连接、电视操控和文字输入；系统建立 `virtual-remote`、`virtual-remote-2` 与 `virtual-search` uinput 设备。Remote stack 分类为 `R2-REMOTE-PASS`；重启后的自动启动和配对持久性未复验。
