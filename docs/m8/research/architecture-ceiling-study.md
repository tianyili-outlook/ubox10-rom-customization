...
Study date: 2026-08-17; build/runtime/source evidence updated: 2026-08-29
...
Accepted rollback/runtime baselines: frozen Android 12 `m8b-remote-r1`, frozen Android 16 ARM32
architecture control `a16-prototype-a-r4`, and frozen Android 16 ARM64 mixed-architecture control
`a16-prototype-b-r7` (**PHYSICAL ARCHITECTURE PASS / PENDING GATE 3 FUNCTIONAL PRESERVATION**);
latest physically accepted kernel checkpoint: `m8-kernel-5.4.302-r5`
...
root-cause closure, bounded r7 offline successor, and exact-r7 physical architecture acceptance.
R1 through r7 have user-supplied UART/runtime evidence; the full r7 raw capture remains outside Git,
and tracked evidence contains sanitized decisive excerpts rather than a fabricated raw path/hash.
...
system_server and Mali-G31 composition. r3 historically required the user's pre-existing runtime
`persist.graphics.egl=mali` override and mapped IR scanCode 352 to UNKNOWN. Strict successor
`a16-prototype-a-r4` contains exactly two source-level functional deltas: read-only
`ro.hardware.egl=mali` while preserving `ro.board.platform=apollo`, and a device-specific
scanCode 352→`DPAD_CENTER` layout. Its 2026-08-26 physical validation now proves fresh boot without
UART/runtime property intervention, source-level EGL selection, Mali/UI, Remote OK, stable HDMI,
Wi-Fi association/DHCP/validated L3, direct audible HDMI output and normal audible VLC media.
...
Exact signed-image and r7 source provenance uniquely proved the r3 ABI chain. Product_a had no scoped
...
authorized only `a16-prototype-b-r4`: source-generated product-scoped mixed metadata intended to preempt
...
status was **OFFLINE CHECKED / READY FOR PHYSICAL VALIDATION**. IMG is 1,641,760,768 bytes / SHA-256
...
R4 physical evidence then proved the correct triplet was written to the wrong storage object.
Runtime `/product -> /system/product`, `/proc/mounts` had no product/product_a mount, and the active
1657-byte embedded build.prop had no triplet; logical product_a existed as dm-1 but was inactive.
Consequently the exact r3 zygote64 ABI abort repeated and r4 is frozen as **PHYSICAL FAIL — PATCHED
INACTIVE LOGICAL PRODUCT_A / RUNTIME PRODUCT SOURCE IS EMBEDDED `/SYSTEM/PRODUCT`**. Exact signed
root, retained `/product` skip-list and first-stage source prove this is the intentional normal-boot
layout. Exact r7 property loading follows the symlink and consumes the embedded build.prop before
the same product→odm→vendor→system derivation. The missing r4 offline invariant was runtime-source
identity, not triplet content.

This uniquely authorized bounded `a16-prototype-b-r5`. It source-generates the canonical triplet in
active `system_a:/system/product/etc/build.prop`, restores inactive product_a to exact r3 bytes, and
changes no ODM/vendor/graphics/kernel/hardware semantics. The signed system tree changes only that
one file. R5 IMG is 1,641,760,768 bytes / SHA-256
...
and the subsequent exact-r7 physical run closes that mapper/runtime gate. Android 16/API36 reports
canonical mixed ABI and `zygote64_32`; both zygotes, ARM64-parented system_server and ARM64
SurfaceFlinger remain running with boot complete. The old `gralloc-mapper is missing` abort does not
recur, real 1920x1080 gralloc allocations appear, and Mali-G31 OpenGL ES 3.2 UI composition is active.
Therefore Android 16 ARM64 mixed architecture on the retained 5.4.302 BSP is **PROVEN VIABLE**.

This freezes exact r7 as the architecture baseline, not a daily-use release. Core retained-ARM32
functional preservation remains pending **Gate 3** on the same image: real H.264/HEVC/VP9 media and
audible HDMI paths, full physical remote matrix, Wi-Fi OFF→ON recovery, basic platform sanity and a
before/after crash census. Gate 3 does not authorize r8 or an image rebuild. The intended future
`codex/m8-a16-development` branch may be derived only after Gate 3 PASS and has not been created.
...
| C — ARM32-only and loaded into a 64-bit consumer | Mali EGL/GLES SP-HAL; HIDL graphics mapper 2.1 and its gralloc implementation; Vulkan driver for 64-bit Vulkan clients | EGL/GLES and mapper are **BOOT-CRITICAL** because AArch64 SurfaceFlinger loads them in-process. Vulkan is **POST-BOOT / APP-CAPABILITY** for B1 because exact r7 defaults RenderEngine to GLES unless a Vulkan flag/backend is selected, and r4 proves GLES. An ELF64 process cannot load any accepted ELF32 provider; HWC itself is not one. |
...
There is also one mandatory non-ELF mixed-mode change: accepted `/vendor/build.prop` owns
`ro.zygote=zygote32` and 32-bit-only vendor ABI lists. Exact r7 init deliberately loads later
partition property files after system and lets later values override earlier ones. A
`zygote64_32` image must therefore update vendor-owned zygote/ABI properties and preserve both
zygote init scripts. `core_64_bit.mk` is genuinely mixed and packages `init.zygote64_32.rc`;
that script imports primary `app_process64` and starts secondary `app_process32`. Merely flashing
its system image cannot override accepted vendor.

Actual r4 AVB inspection further bounds the change: vendor has its own hashtree plus signed
`vbmeta_vendor.fex`; system has its own hashtree plus signed `vbmeta_system.fex`; top-level
`vbmeta.fex` has no chain descriptor. A first mixed candidate therefore expects system, vendor,
super, both subordinate vbmeta payloads and their Allwinner `V*` companions to change, while
top-level vbmeta should remain byte-identical if the verified topology is preserved.
...
The earlier study used an ignored donor workspace under `/home/tianyi/ubox10-ceiling-donors`.
That path is not present on the current VM. The 2026-08-26 revalidation therefore used the official
BPI GitHub commit/tree and temporary read-only copies of only the three Mali files needed for
ELF/hash inspection. No donor host binary was executed, no donor file was copied into a candidate,
all temporary proprietary copies were removed after inspection, and no proprietary binary is
...
The earlier retained-donor review recorded a GPU notice describing the binary stack as
confidential/proprietary and subject to an Allwinner agreement; the current public repository has no
top-level license that independently grants redistribution of those binaries. Public Git
availability is therefore **not** a proven right to redistribute. The gralloc source carries a
separate Apache-2.0 notice.

The exact public source identity is commit
`316cd80ca43fa17b0385eacd7f6f3652bbd66b2a`. The donor ARM32 file at
`hardware/aw/gpu/mali-bifrost/mali-g31/arm/lib/libGLES_mali.so` is 14,487,716 bytes /
`fbffe5601a58d1f8d624ee37129f73b76d0a73eb21fc8a2487368d9ab47f14b7`, byte-identical to
accepted `/vendor/lib/egl/libGLES_mali.so`. The paired
`arm64/lib64/libGLES_mali.so` is 18,145,112 bytes /
`03333d495e3566c7d85ca2e000da569a16ce8f022ea25c0ea61950c891d5c7f8`, ELFCLASS64,
AArch64 and SONAME `libGLES_mali.so`. Its DT_NEEDED set is `libcutils.so`,
`android.hardware.graphics.common@1.0.so`, `libnativewindow.so`, `liblog.so`, `libm.so`,
`libc.so`, `libdl.so`, `libz.so`, and `libc++.so`; exact r7 VNDK31/LLNDK arm64 prebuilts export
...
provenance uniquely closed the r3 cause, but bounded r4 placed its triplet in runtime-inactive
logical product_a and physically failed unchanged. Exact runtime-source proof then authorized
bounded r5, which puts the triplet in active embedded `/system/product` and closes all offline
gates. R5 now awaits physical ABI validation; graphics remains separately unresolved.
...
At the offline-closure point this was **OFFLINE CHECKED / READY TO REQUEST PHYSICAL
VALIDATION**, not a physical pass. HDMI, audio, Wi-Fi and Ethernet were unchanged. Exact artifact
and preservation evidence is in `docs/m8/candidates/a16-prototype-a-r4.md` and its preservation
JSON. The later physical result below supersedes only the pending validation status, not the
two-delta implementation history.

#### Prototype A r4 physical closure and formal Gate 2 adjudication

The user physically flashed and tested exact r4 on 2026-08-26. The original raw ADB/log captures
were not present on this VM, so the tracked evidence record explicitly identifies external user
physical confirmation rather than fabricating raw files or hashes:
`docs/m8/device-tests/20260826-a16-prototype-a-r4-physical-validation/`.

Fresh r4 boots Android 16/API36/`zygote32` to `sys.boot_completed=1` without UART, bootarg,
runtime `setprop` or persistent EGL workaround. `persist.graphics.egl` is empty,
`ro.hardware.egl=mali`, `ro.board.platform=apollo`; Mali-G31, SurfaceFlinger and the UI pass.
InputManager selects `sunxi-ir.kl`, and physical KEY_OK scanCode 352 dispatches as
`DPAD_CENTER(23)`; normal remote navigation passes. Both r4 source deltas are therefore physically
proven.

HDMI is physically stable and r3's black cycle is not reproduced, but r4 changed no display code,
so the old root cause is not proven. Wi-Fi modules, scan, association, WPA completion, DHCP, IPv4,
DNS, Android validated/trusted Internet and real use pass. A Wi-Fi OFF→ON script disconnected its
own Wi-Fi ADB transport and is recorded NOT COMPLETED, not FAIL; prior kernel-r5 evidence separately
proves reinitialization. Ethernet has no current-session carrier and is NOT RETESTED; exact
preservation plus prior physical PASS remain the control.

The user heard a real 48 kHz/16-bit/stereo WAV through ALSA `ahubhdmi` and HDMI TV speakers. ARM32
VLC produced normal H.264/AAC-style video and audible TV audio through AudioFlinger/vendor HIDL/
ALSA. Audio service PIDs stayed stable and the post-logcat-clear playback interval had an empty
crash buffer. Functional and steady-state playback pass.
...
failure. Strict r7 closes only that pair. Exact-r7 physical evidence now proves canonical mixed ABI,
dual zygote, ARM64-parented system_server, stable ARM64 SurfaceFlinger, real gralloc allocation and
Mali-G31 GLES/UI. The mapper gate is **PHYSICAL PASS**, and exact r7 is the frozen Android 16 ARM64
mixed-architecture architecture baseline pending Gate 3 functional preservation.
...
| Target C — Modern Hybrid | **r7 PHYSICAL ARCHITECTURE PASS / PROVEN VIABLE / GATE 3 PENDING** | API 36 plus AArch64 apps while preserving process-isolated 32-bit HALs/kernel | Exact r7 boots with mixed ABI, dual zygote, ARM64-parented system_server, stable ARM64 SurfaceFlinger, mapper/gralloc allocation and Mali-G31 UI | High but bounded | Freeze r7 architecture; execute exact-r7 functional-preservation Gate 3 before productization |
...
**Overall confidence: HIGH for ARM32 architecture viability, r4 functional integration, formal
Prototype A closure and mixed-architecture feasibility; functional preservation remains pending.** The r1/r2/r3 progression proves
first stage through ARM64 second-stage runtime, kernel r5 closes the same-lineage 5.4.302 wireless checkpoint, and exact r7/B-r3-r5 outputs
close the bounded offline contracts. Prototype A r4 physically proves no-runtime EGL, Remote OK, stable HDMI,
...
not an independent zygote crash, prevents system_server. Exact r7 lookup/linker/control evidence
closed the mapper root cause, and exact-r7 physical evidence now closes the two-file instantiation
contract plus stable SurfaceFlinger/system_server/Mali UI architecture gate. Confidence in retained
media/input/network/storage functionality under this mixed runtime remains bounded until Gate 3.
...
| Mixed ARM64/ARM32 userspace | **R7 PHYSICAL ARCHITECTURE PASS / PROVEN VIABLE / FROZEN** | r5 proves active embedded-product/global mixed ABI; r6 crosses BoringSSL; exact r7 runs dual zygotes, ARM64-parented system_server and stable ARM64 SurfaceFlinger, with mapper/gralloc allocation and Mali-G31 UI. Gate 3 functional preservation remains. |
...
4. **Completed architecture chain / Gate 3 pending:** r2 passes r1 `/metadata`, r3 passes r2
   ...
   SurfaceFlinger crash-health then kills zygote. Strict r7 closes only the proven mapper/gralloc
   relocation pair, and exact-r7 physical evidence proves mapper allocation, Mali UI, stable
   SurfaceFlinger, dual zygote and ARM64-parented system_server. Remaining P0 is exact-r7 Gate 3
   functional preservation, not another architecture candidate.
...
15. **Completed — freeze and explain r4 physical failure:** r4 preserved prior boot boundaries but
...
18. **Completed — exact single-cause r7 architecture acceptance:** source/image/runtime audit proves mapper discovery metadata
    correct and identifies the sole ARM64 mapper/gralloc VNDK31 libc++ relocation incompatibility.
    Only that pair uses the documented ARM64 back-deploy hook; full offline audit passes. Physical
    validation closes `gralloc-mapper is missing` and proves mixed ABI, dual zygote, system_server,
    stable SurfaceFlinger, gralloc allocation and Mali-G31 UI. Exact r7 is frozen architecture control.
19. **Current — Gate 3 functional preservation:** on exact r7 only, reconfirm architecture and test
    real H.264/HEVC/VP9 media plus HDMI audio, full remote keys, Wi-Fi OFF→ON recovery, basic platform
    sanity and before/after crash census. This is read-only physical validation, not r8 or a rebuild.
20. **After Gate 3 PASS:** freeze the Android 16 functional baseline, then derive
    `codex/m8-a16-development` for stabilization/productization. Enforcing, full-VINTF NFS cleanup,
    audio P1, product features and broader release hardening remain separate work.

## 17. Sources and provenance

Exact official r7 tag objects were audited locally on 2026-08-24. BPI provider/source identity was
revalidated against the official repository on 2026-08-26. Primary/official sources are used for
platform, kernel and board claims.
...
- [BPI H618 Android 12 public source at revalidated commit](https://github.com/BPI-SINOVOIP/BPI-H618-Android12/tree/316cd80ca43fa17b0385eacd7f6f3652bbd66b2a)
...
ELF inventory, prior live read-only ADB captures, the read-only Android 12 UBOX product, current
repository candidate/device-test records and the isolated Android 16 tree. The prior donor
workspace was absent on 2026-08-26; current donor facts were revalidated directly against the
official Git commit and temporary targeted files were removed after static inspection. The Linux
5.4.302 checkpoint additionally
