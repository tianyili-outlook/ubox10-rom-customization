...
活跃架构开发位于 `codex/m8-architecture-ceiling` 的 Android 16 路线；决策依据为该分支的 `docs/m8/research/architecture-ceiling-study.md`。长期 mixed AArch64-primary / ARM32-secondary 目标继续 **HOLD**，Prototype B 不得启动。A16 r2 实机已推进到 r4/25Q4 的 `NetBpfLoad` 5.10 硬门槛，Path A 因此选用早期 A16 QPR0 + final 5.4 lineage。Android 12 `m8-kernel-5.4.302-r1` 物理测试证明 Linux 5.4.302、`sys.boot_completed=1`、HDMI/UI、遥控、Ethernet 与 ADB 正常，但 Wi-Fi 在 AIC8800D firmware START_APP 1037→1038 handshake 可重复超时；r2 已实机拒绝 50 MHz hypothesis。r3 证明 final 512-byte CMD53 write 在 Linux host 返回 0 后没有 AIC IRQ/RX/1038/completion；r4 又实机证明 timeout 时 function 1、hardware SDIO IRQ、claim/handler 与 `IENx=0x03` 正常，但 `INTx=0x00`、core pending=0，故没有 persistent function pending indication 保留到 snapshot。当前精确边界为 **HOST/CONTROLLER-COMPLETE CMD53 → DEVICE FIFO DEQUEUE/PARSE**；不证明无 transient signal、device consumption 或 firmware failure。Exact U04 FMAC binary audit 找到 post-start 1037→1038 producer，但 exact boot-ROM consumer/AUTO handoff 仍缺失，r5 无 source-proven safe discriminator。same-lineage preservation 仍为 **PARTIAL PHYSICAL PASS / WIRELESS OPEN**，Gate 2 保持 **CLOSED**，没有形成 A16 r3。
...
没有找到 exact U04 read-only dequeue pointer、boot latch、CPU PC/status、FMAC-ready magic、exception state 或 CFM-constructed bit。`bootstatus` 只在缺失的 1038 内存在并被 host 当作 `hwinfo_r`；`host_ready` string 无 published address/read contract；different-chip FNCALL/DUMMY/COMREG 不适合作为 U04 discriminator。因此 **r5 = NO / NOT JUSTIFIED**。最早未解边界保持为 host/controller-visible CMD53 completion → device FIFO dequeue/request parsing；若 request 已消费，下一未解段为 boot-ROM AUTO handoff → reset-vector execution → exact FMAC 1037 handler/1038 send。详见 `docs/m8/candidates/m8-kernel-5.4.302-r4.md`。
...
- 当前 board、DT 与 runtime 证据识别为 H616。A16 ARM32 r2 稳定失败于 r4/25Q4 NetBpfLoad 的 5.10 门槛。Path A 的 5.4.302 r1 已物理证明 Android 12 boot 与多项板级合同，但 AIC8800D Wi-Fi START_APP handshake 仍失败；r2 已实机拒绝 50 MHz hypothesis，r3 已实机把边界收敛为 host-complete TX 后无 AIC handler/RX/1038，r4 又证明 timeout 时 IRQ enable/claim/handler 正常但无 persistent function pending。Exact FMAC audit 找到 post-start 1037→1038 producer，却没有 exact boot-ROM handoff 或安全 device-state discriminator；r5 不成立。在 wireless boundary 获得新的 authoritative artifact 前不构建 A16 r3、不启动 AArch64 Prototype B。
...
保持 `m8b-remote-r1`、Test8r2、stock rollback、A16 r1/r2 与 kernel r1/r2/r3/r4 artifacts 不变。r4 physical result 已完成，当前不构建 r5，也不执行新的物理动作。只有取得 exact AIC8800D U04 boot-ROM source/image+verified map、vendor START_APP/boot-flow specification，或 exact working U04 device-side trace，并从中得到 source-proven read-only dequeue/boot/FMAC-ready discriminator，才重新评估 software-only experiment。Wireless checkpoint 关闭后才进入 `android-security-16.0.0_r7` source-only audit；不构建 A16 r3、不启动 Prototype B，Gate 2 保持 **CLOSED**。
