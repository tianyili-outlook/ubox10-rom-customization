...
- 当前 board、DT 与 runtime 证据识别为 H616。A16 ARM32 r2 稳定失败于 r4/25Q4 NetBpfLoad 的 5.10 门槛。Path A 的 5.4.302 r1 已物理证明 Android 12 boot 与多项板级合同，但 AIC8800D Wi-Fi START_APP handshake 仍失败；r2 已实机拒绝 50 MHz hypothesis，r3 已实机把边界收敛为 post-TX/pre-AIC-handler。r4 仅加入 timeout 后 CCCR pending/enable snapshot，已离线审核但未授权或执行物理测试；在该 boundary 收敛和后续 r7 source audit 前不构建 A16 r3、不启动 AArch64 Prototype B。
...
保持 `m8b-remote-r1`、Test8r2、stock rollback、A16 r1/r2 与 kernel r1/r2/r3/r4 artifacts 不变。当前精确下一动作是等待用户对 r4 exact hash `18565E4F94FF1A843EA859254800E5E2BA732FBFE47410E86D6577038F85DFCA` 的单独明确物理授权；若授权，只做一次 UART-first diagnostic 和一次手动 Wi-Fi ON，读取 `AIC_STARTAPP_TRACE` 的 CCCR `INTx/IENx`、core pending、IRQ claim/handler state，并保持既有 r3 TX/IRQ/RX/dispatch 字段。不得在同次测试加入修复；INTx clear 不能单独宣称 firmware failure。Wireless checkpoint 关闭后才进入 `android-security-16.0.0_r7` source-only audit；不构建 A16 r3、不启动 Prototype B，Gate 2 保持 **CLOSED**。
