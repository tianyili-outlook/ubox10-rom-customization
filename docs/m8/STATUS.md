...
最早 source-proven divergence 已前移到 `aicwifi_init()` 的 FMAC upload destination，修订边界为 wrong image placement/boot address → boot-ROM AUTO handoff → FMAC reset-vector execution；r4 的 CMD53-complete → FIFO-dequeue 观察缺口仍存在，但不再是最早已知差异。**r5 = YES, DESIGN ONLY / NOT BUILT**：未来若另行授权，只把 `RAM_FMAC_FW_ADDR` guard 改为实际定义的 `AICWF_SDIO_SUPPORT`，保留 r4 trace，并在 final ELF 验证 upload、patch read 与 START_APP 都为 `0x00120000`。这不是 Wi-Fi PASS 或 accepted fix；Gate 2 仍 **CLOSED**。详见 `docs/m8/candidates/m8-kernel-5.4.302-r4.md`。
...
- 当前 board、DT 与 runtime 证据识别为 H616。A16 ARM32 r2 稳定失败于 r4/25Q4 NetBpfLoad 的 5.10 门槛。Path A 的 5.4.302 r1 已物理证明 Android 12 boot 与多项板级合同，但 AIC8800D Wi-Fi START_APP handshake 仍失败；r2 已实机拒绝 50 MHz hypothesis，r3 已实机把边界收敛为 host-complete TX 后无 AIC handler/RX/1038，r4 又证明 timeout 时 IRQ enable/claim/handler 正常但无 persistent function pending。Exact FMAC audit 找到 post-start 1037→1038 producer；accepted-driver audit 又证明 r1-r4 错用 `0x00110000`，与 working BSP 和 exact FMAC 的 `0x00120000` contract 不符。该差异使 r5 单变量 design 成立但尚未构建/实测；在另行授权前不构建 A16 r3、不启动 AArch64 Prototype B。
...
保持 `m8b-remote-r1`、Test8r2、stock rollback、A16 r1/r2 与 kernel r1/r2/r3/r4 artifacts 不变。本轮只完成 audit 与 r5 design；没有 build、driver/firmware 修改或物理动作。若用户另行授权，唯一合理软件动作是在 r4 上把 `RAM_FMAC_FW_ADDR` guard 从未定义的 `CONFIG_AIC_INTF_SDIO` 改为实际 build define `AICWF_SDIO_SUPPORT`，并以 final ELF 锁定 upload/patch-read/START_APP 的 `0x00120000` 单变量合同；授权前不得构建或刷写。Wireless checkpoint 关闭后才进入 `android-security-16.0.0_r7` source-only audit；不构建 A16 r3、不启动 Prototype B，Gate 2 保持 **CLOSED**。
