...
- 当前 board、DT 与 runtime 证据识别为 H616。A16 ARM32 r2 稳定失败于 r4/25Q4 NetBpfLoad 的 5.10 门槛。Path A 的 5.4.302 r1 已物理证明 Android 12 boot 与多项板级合同，但 AIC8800D Wi-Fi START_APP handshake 仍失败；r2 已实机拒绝 50 MHz hypothesis。下一步只允许先做 START_APP-gated AIC transport instrumentation 的单变量离线候选设计/审核；在 wireless compatibility 收敛和后续 r7 source audit 前不构建 A16 r3、不启动 AArch64 Prototype B。
...
保持 `m8b-remote-r1`、Test8r2、stock rollback、A16 r1/r2 与 kernel r1/r2 artifacts 不变。当前精确下一动作是：设计并 source-review 一个只改变 `aic8800_bsp.ko` 可观测性的 START_APP-gated diagnostic，记录 1037 final CMD53 TX return、AIC IRQ/block count、CMD53 RX length/header/id 以及 1038 dispatcher/token match；不得改变 firmware、DT、userspace/HAL、generic timeout 或行为。只有该 instrumentation 的单变量边界与离线 preservation audit 关闭后才可形成候选；任何实机仍需重新明确授权。Wireless checkpoint 关闭后才进入 `android-security-16.0.0_r7` source-only audit；不构建 A16 r3、不启动 Prototype B，Gate 2 保持 **CLOSED**。
