...
- [x] **33 — Gate 3 — Android 16 Mixed-Architecture Functional Preservation（`PASS_WITH_EXPLICIT_USER_WAIVER` / CLOSED）：**只在
  ...
  `R8_AUDIT_DECISION = HOLD_FOR_MORE_EVIDENCE`；diag1a随后补齐paired usage/planes/AFBC/import与首个
  EGL error，但仍不自动授权repair。Exact-r7-derived
  ...
  **BOOT FAIL / ROOT CAUSE PROVEN / CLOSED**：ARM32 gralloc强导入 retained VNDK31 libc++未导出的
  ...
  back-deploy；它此前达到 **OFFLINE CHECKED / READY FOR PHYSICAL BOOT GATE**，boot PASS之后的
  paired/live证据现已证明HEVC首失败为 `eglCreateImageKHR` / `EGL_BAD_ALLOC 0x3003`，
  且AVC/HEVC allocation/import contract基本一致。首个明确语义差异为port-settings change后AVC crop保持
  1920x1080、HEVC crop提升为1920x1088。单变量 `a16-prototype-b-r7-diag2-hevc-crop`只在exact HEVC/YV12/
...
- [x] **P3-A — PHYSICAL FAIL；original RC-A PHYSICAL REPAIR EFFECTIVE；RC-A2/RC-B FORENSICS COMPLETE：**`a16-dev-p3a-omx-r1`实测不再出现原`__anDrain` NULL，正式路径已到FBD/SF。preparse另暴露RC-A2：internal FBM `pMetaData`只分配4 KiB而HEVC写23,480 bytes，首次free时Scudo发现heap header损坏；结论`READY_FOR_NARROW_BINARY_PATCH`。正式播放精确3840x2160 YV12 buffer `9891309682708` / backing store `2229088026704`采用19,489,120-byte auto-AFBC-big private contract，compat1a不匹配后在Mali复现crop ABI rejection；同metadata collision为very-high-confidence，compat1b为`READY_FOR_EXACT_COMPAT1B_IMPLEMENTATION`，但仍可能存在翻译后的独立4K import limit。RC-A2现已单独落实为`a16-dev-p3a-fbm-r1`：internal FBM allocation `0x1000`→`0x6000`，唯一runtime delta为`/vendor/lib/libfbm.so`的2字节；**OFFLINE CHECKED / PHYSICAL VALIDATION PENDING**。compat1b未实施。候选合同见`docs/m8/device-tests/20260905-a16-p3a-fbm-r1-build/README.md`。详见`docs/m8/device-tests/20260905-a16-p3a-rca2-compat1b-forensics/README.md`。
