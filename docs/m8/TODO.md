...
- [x] **P3-A — PHYSICAL FAIL / FORENSICS COMPLETE：**一次HEVC Main 8-bit SDR 3840x2160p30 bounded attempt出现两个独立边界。首先exact ARM32 `libOmxVdec.so::__anDrain+1212`在后续`RequestPicture`填充前解引用NULL current `VideoPicture*`；OMX service单独重启后，正式播放又把3840x2160 YV12送到SF，但compat1a exact 1080p predicate必然排除4K，original view产生invalid Ganesh texture并触发SF/zygote userspace restart。4K fixed sidecar使已知metadata ABI collision成为graphics首要假设，但缺失exact 4K usage/planes/sidecar/EGL error，仍不能排除独立4K import limit。下一顺序：先取得matching OMX source或等价state证据并修复drain lifecycle，再单次复验并据真实4K contract决定compat1b；当前不实施任何修复。
