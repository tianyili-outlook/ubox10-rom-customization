...
- [ ] **当前唯一下一动作：**设计并 source-review 一个 START_APP-gated、diagnostic-only `aic8800_bsp.ko` 单变量 instrumentation，记录 1037 final CMD53 TX return、AIC SDIO IRQ/block count、CMD53 RX return/length/header/message ID、1038 dispatcher/token match。不得改变 timeout、firmware、DT、userspace/HAL 或 generic MMC behavior；通过离线 preservation audit 后才可形成新的 Android 12 kernel-only diagnostic candidate，任何物理测试仍需重新明确授权。
