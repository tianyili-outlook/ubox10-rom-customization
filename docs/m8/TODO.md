...
- [x] **P3-A — PHYSICAL FAIL / FORENSICS COMPLETE；RC-A PATCH IMPLEMENTED OFFLINE / CANDIDATE BUILT：**一次HEVC Main 8-bit SDR 3840x2160p30 bounded attempt出现两个独立边界。RC-A source archaeology证明copy path在`RequestPicture`取得ownership前错误读取NULL current-display slot；accepted readiness basis为`READY_FOR_NARROW_BINARY_PATCH`。`a16-dev-p3a-omx-r1`仅将该operand改为已NULL-check的`NextPictureInfo` non-owning peek。相对`a16-dev-audio-r1`的semantic filesystem delta严格为`/vendor/lib/libOmxVdec.so`，仅3字节改变；离线检查完成，**PHYSICAL VALIDATION PENDING**。P3-A不因此成为PASS；下一步仍是一轮受控Main 8-bit SDR 4K30复验，采集exact 4K contract后才决定RC-B/compat1b。graphics侧仍不能排除独立4K import limit。
