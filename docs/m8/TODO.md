...
- [ ] **4K non-surface thumbnail physical validation：**补充raw日志已确认内部AFBC picture被线性YV12→I420 memcpy误读。`a16-dev-p3a-thumbnail-r1`已构建并离线检查，仅对exact HEVC3840x2160/0x13、非native/zero-copy/secure CPU合同在初始化前关闭内部压缩。下一步需另行授权新文件缩略图、1080控制及一次有界4K Surface回归；不能用旧缓存判PASS。当前记录：`docs/m8/device-tests/20260906-a16-p3a-thumbnail-r1-build/README.md`。RC-A2/audio P1 CLOSED、P2 COMPLETE、r8未授权不变；本次未操作设备。
