...
- [ ] **当前唯一下一动作：**本轮不构建 r5、不修改 driver/firmware、不执行物理动作。若用户另行授权，按已锁定 design 在 r4 上只把 `RAM_FMAC_FW_ADDR` guard 改为 `AICWF_SDIO_SUPPORT`，保留 r4 trace，并以 final ELF 验证 upload、patch read、START_APP 三处均为 `0x00120000`。**r5 = YES / DESIGN ONLY / NOT BUILT**；仍不是 accepted fix，Gate 2 保持 CLOSED。
