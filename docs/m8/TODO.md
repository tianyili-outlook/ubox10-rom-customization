...
- [x] **P3-A — PHYSICAL FAIL / FORENSICS COMPLETE；RC-A READY_FOR_NARROW_BINARY_PATCH：**一次HEVC Main 8-bit SDR 3840x2160p30 bounded attempt出现两个独立边界。RC-A source archaeology已用near-exact Allwinner wrapper/FBM源码与exact binary证明：copy path在`RequestPicture`取得ownership前错误读取NULL current-display slot；正确语义是从已NULL-check的`NextPictureInfo` non-owning peek读取color aspects，且不是4K-specific guard。当前仅完成patch design，未修改ELF；修复顺序仍为OMX first，再单次复验并采集exact 4K contract后决定RC-B/compat1b。graphics侧仍不能排除独立4K import limit。
