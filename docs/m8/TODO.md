...
- [ ] **1 — bounded graphics delta：**下一轮 GCP candidate 加入 `ro.hardware.egl=mali`，严格保留 `ro.board.platform=apollo`，不得依赖 runtime `setprop`；本 handover 未实现、未构建、未物理验证该变化。
- [ ] **2 — bounded input delta：**只把 sunxi-ir scanCode 352 映射到 DPAD_CENTER，保持其他已验证按键映射不变；当前 Android OK mapping 仍为 FAIL，Android keyCode 仍为 UNKNOWN。
- [ ] **3 — HDMI boundary：**基于当前 3840x2160 YUV444 mode 34 / 60 Hz evidence，先收敛 timing/link/receiver-lock boundary；framework/extcon/display counters 未解释约 1 秒画面/约 5 秒黑屏，不在证据不足时任意替换 display stack，也不声称 root cause proven。
- [ ] **4 — audio boundary：**调查 HDMI transition 进入 legacy HIDL `getAudioPort` null-pointer SIGSEGV 的 exact source path；当前只把 null callback/function pointer 记为 **HIGH CONFIDENCE / LIKELY**，source-level root cause **NOT PROVEN**。后续须使用有音频输出的 HDMI sink 完成 basic/HDMI 实际听音。
- [ ] **5 — Wi-Fi closure：**保持当前 AIC BSP/modules、HAL 与 firmware；下一轮安排可输入凭据的 association、DHCP、validated L3 与 DNS-over-Wi-Fi 验证。当前只有 framework enable、scan 与 OFF→ON reinitialization PASS。
- [ ] **6 — candidate acceptance：**后继候选必须无需 runtime EGL intervention，并通过稳定 HDMI、physical remote OK、Wi-Fi association、vendor audio HAL stability 与真实 audio sink 听音；保留 rollback 与 exact hash，完成离线审核后再请求独立物理授权。
- [ ] **7 — architecture boundary：**Gate 2 继续 **NOT CLOSED**。Prototype B、`zygote64_32`、secondary ABI 与 ARM64 Mali/mapper integration 在无 runtime intervention 的 Prototype A 完成 Gate 2 前保持 **CLOSED**。
