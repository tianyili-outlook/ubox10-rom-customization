...
- [x] **P3-0 RESEARCH / TOOLING PREPARED：**retained H616 DT/kernel证明4路THS、CPU/GPU cooling与70/90/115°C CPU trips，但校准有效性和normal-shell sysfs可读性仍须只读实机probe，故结论为 **PARTIAL OBSERVABILITY — SHORT SMOKE ONLY**。动态thermal/cooling/CPUfreq/devfreq observer及单次短时P3-A合同已就绪；P3-A 8-bit SDR 4K30仍 **PHYSICAL CAPTURE PENDING**，未执行。
- [ ] **P3-A — HEVC Main 8-bit SDR 3840x2160p30：**仅在read-only discovery证明plausible CPU temperature与CPUfreq可读后，执行一个≤30秒fixture/≤60秒sampling的手工single-play bounded smoke；必须分层判定bitstream→OMX/Cedar→buffer→EGL/HWC/SF→visible/audio/system/thermal。Compat1a exact predicate排除4K，不能预设metadata ABI shadow会激活。
