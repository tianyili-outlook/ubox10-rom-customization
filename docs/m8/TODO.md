...
- [ ] **唯一后续候选动作：**在 GCP 构建前锁定 bounded delta：保持 `ro.board.platform=apollo` 并加入 `ro.hardware.egl=mali`；映射 sunxi-ir scanCode 352→DPAD_CENTER；基于 4K60 YUV444/link evidence 与 HIDL `getAudioPort` transition evidence 决定最小 HDMI/audio change。Wi-Fi BSP/HAL 保持，另安排可输入凭据的 association validation。任何 image 仍需离线审核与单独物理授权；Gate 2 **NOT CLOSED**，Prototype B/`zygote64_32`/secondary ABI/ARM64 Mali-mapper 继续关闭。
