...
- [ ] 准备至少 400 GB 可用的 Linux/ext4 构建卷。
- [ ] 记录构建路径、实际空闲空间和清理方式。

## P1：补轻量运行时快照

以下可在准备构建卷时通过 ADB 完成；缺少其中单项不阻塞 M8A.2a：

- [ ] 保存 `/linkerconfig/`。
- [ ] 保存活动 APEX、bootclasspath 和 system_server classpath。
- [ ] 保存 uses-library 与一次基本 `checkvintf` 结果。
- [ ] 若切回官方 ROM 很方便，在首次 M8A 实机 candidate 前补 DRM 对照。

## P2：M8A.2a 离线 product
...
已完成：M8 当前设备 inventory、Test8r2 DRM 基线、Android 12 source-lock、
M8A product 差异、分区预算和产品计划。
