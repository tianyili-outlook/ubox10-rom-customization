...
- [x] Source-review 证明 current r3 无安全、可归属的 card CCCR pending readout；构建并离线审核 `m8-kernel-5.4.302-r4`。它只在原 timeout 后、teardown 前读取 CCCR `INTx`/`IENx` 并记录 host/core/claim/handler state，保持 r1/r3 70 MHz、timeout 前控制流与所有非 BSP bytes。AIC export CRC 已恢复为 r1 byte-identical；AVB/ext4/LP/IMAGEWTY/single-module checks PASS。Candidate 为 1,031,739,392 bytes / SHA-256 `18565E4F94FF1A843EA859254800E5E2BA732FBFE47410E86D6577038F85DFCA`，状态 **OFFLINE CHECKED / INSTRUMENTATION ONLY / NOT A FIX / NOT PHYSICALLY VALIDATED**。
- [ ] **当前唯一下一动作：**等待对上述 exact r4 hash 的单独明确物理授权；若授权，只执行一次 UART-first diagnostic 与一次手动 Wi-Fi ON，读取 CCCR pending/enable、core pending、IRQ claim/handler state 及既有 r3 TX/IRQ/RX/dispatch 字段。不得在同次测试加入修复；INTx clear 不能单独宣称 firmware failure；Gate 2 保持 CLOSED。
