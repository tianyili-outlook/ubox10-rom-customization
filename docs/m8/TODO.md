...
- [ ] **下一会话唯一授权架构动作：**按 `docs/m8/research/android-16-qpr0-r7-source-audit.md` 的 exact contract 构建并完整离线审核一个 ARM32/no-secondary/`zygote32` Prototype A r3。当前任务未构建 r3；Gate 2 是 **UNBLOCKED / READY FOR QPR0 r3 BUILD**，不是 PASS。Prototype B、`zygote64_32`、ARM64 Mali/mapper integration 与任何 physical flash/test 仍关闭。
