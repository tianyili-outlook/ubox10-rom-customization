...
- **M8A 只继承 AOSP ATV 产品层**：采用锁定版本的
  `atv_generic_system.mk`、`atv_system_ext.mk`、`atv_product.mk` 语义，
  不采用 emulator、goldfish 或 generic_x86 vendor；UBOX10
  boot/kernel/vendor/vendor_dlkm/TEE 与 display VINTF 保持原合同。
- **M8A 首版保留 AwTvProvision**：先保留其可选 Device Owner provisioning，
  基础启动后再单变量替换为 AOSP TvProvision；SettingsSetup、AwManager 和
  PackageOverride 不迁移。
- **完整 AOSP 不下载到当前磁盘**：准备至少 400 GB 可用的 Linux/ext4
  构建卷后再同步，不把大型源码树放入本仓库或当前 WSL VHDX。
