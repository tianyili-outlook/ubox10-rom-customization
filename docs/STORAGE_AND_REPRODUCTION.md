...
| 当前实验候选 | `out/candidates/test9w1-disable-aic-ant-div-r1/x12-test9w1-disable-aic-ant-div.img` | `2D43D4A6B64702F1D0265EDC27B33EB424B4B56A721DC8068B5CCEBB4A310CC5` |
...
- `work/preinstall_apks/`：当前候选和后续用户态安装所需、未提交 Git 的第三方官方 APK。
...
- 所有淘汰候选目录及其固件和中间分区。
- Test8r2/Test9w1 目录内除最终 PhoenixCard 固件外的 `system_a/super/vendor/product/vbmeta` 等中间 `.img`。
...
第一条恢复并验证官方构建输入；第二条事务式重建指定候选。第三方 APK 必须按配置中的来源和 SHA-256 放回 `work/preinstall_apks/`。

## 清理记录

- 清理前：149 个 `.img`，合计约 85.677 GiB；`out/candidates/` 约 70.811 GiB，`work/` 约 8.506 GiB。
- 清理后：只剩上表三份 `.img`，合计 6,030,742,528 bytes（约 5.617 GiB）；`.img` 占用减少约 80.060 GiB。
